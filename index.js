  ...
  parse(tokens) {
    ...
    while (pos < len) {
      ...
      if (token.type === "keyword" && token.value === "print") {
        ...
        if (!isString && !isVar) {
          return console.log(
            `Unexpected token ${tokens[pos + 1].type}, expected string`
          );
        }
        if (isVar) {
        ...
        const isCustomKW =
          tokens[pos + 1] && tokens[pos + 1].type === "keyword_custom";
        if (!isCustomKW) {
          if (!tokens[pos + 1]) {
            return console.log(
              "Unexpected end of line, expected variable name"
            );
          }
          return console.log(
            `Unexpected token ${tokens[pos + 1].type}, expected variable name`
          );
        }
        ...
        const isEq =
          ...
          tokens[pos + 2].value === "eq";
        if (!isEq) {
          if (!tokens[pos + 2]) {
            return console.log("Unexpected end of line, expected =");
          }
          return console.log(
            `Unexpected token ${tokens[pos + 1].type}, expected =`
          );
        }
